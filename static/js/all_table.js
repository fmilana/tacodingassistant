/* global document d3 screen window regexp themes reclassifyBackend threadStartId logBackend $*/
...
const allTableLib = (function () {
  ...
  const generateTable = function () {
      ...
      for (let i = 0; i < themes.length; i++) {
        ...
        for (let j = 0; j < data.length; j++) {
          ...
          if (dataRow[theme] !== null) {
            ...
            themeDataRow.push(dataRow[theme][1]); // predict sentences
            themeDataRow.push(dataRow[theme][2]); // train sentences
    ...
    const table = d3.select('#all-table-container')
      .append('table')
      .attr('class', 'green')
      .classed('center', true);
          ...
          if (predictSentences.length > 0 || trainSentences.length > 0) {
            ...
            if (predictSentences.length > 0) {
              for (let j = 0; j < predictSentences.length; j++) {
                ...
                if (j === (predictSentences.length - 1)) {
    ...
    const titleRect = d3.select('#all-table-container')
  ...
  const keywordDragged = function (event) {
    if (!d3.select('#all-table-container').select('#bin-div').classed('expanded')) {
      d3.select('#all-table-container')
        .select('#bin-div')
  ...
  const keywordDragEnded = function (event) {
    if (dragging) {
      ...
      if (targetColumn !== movingColumn) {
        ...
        logBackend.log(`[${dateTime.toLocaleString()} (${msTime})]: keyword "${movingText}" (${movingColumn}) at position ${d3.select(this.parentNode.parentNode).attr('position')} moved to "${targetColumn}"`);
  ...
  const sentenceDragEnded = function (event) {
    if (dragging) {
      ...
      if (targetColumn !== movingColumn) {
        ...
        let msTime = new Date().getTime();
        let dateTime = new Date(msTime);
        logBackend.log(`[${dateTime.toLocaleString()} (${msTime})]:: ${logString} sentence moved to "${targetColumn}"`);
        // setTimeout to avoid freezing
        setTimeout(() => {
      ...
      .each(function () {
        ...
        d3.select(this)
          .on('click', function () {
            let msTime = new Date().getTime();
            let dateTime = new Date(msTime);
            logBackend.log(`[${dateTime.toLocaleString()} (${msTime})]:: keyword "${d3.select(this).text()}" (${d3.select(this).attr('column')}) at position ${d3.select(this.parentNode.parentNode.parentNode).attr('position')} clicked`);
